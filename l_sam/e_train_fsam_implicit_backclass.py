...
from l_sam.forveated_sam.foveated_sam_implicit_backclass import build_foveated_sam
...
if __name__ == '__main__':
    ...
    old_state_dict = torch.load("/root/autodl-tmp/DynamicFocus/l_sam/efficient_sam_vitt.singlemask.pt")['model']
    ...
    if True:
        ...
        all_mask_decoder_params = list(efficientsam_ti_custom.mask_decoder.parameters())

        classify_model_params = list(efficientsam_ti_custom.mask_decoder.classify_model.parameters())
        classify_model_param_ids = set(id(p) for p in classify_model_params)

        filtered_mask_decoder_params = [p for p in all_mask_decoder_params if id(p) not in classify_model_param_ids]


        optimizer = torch.optim.Adam(
            [
                {"params": filtered_mask_decoder_params, "lr": lr1},
        ...
        for epoch in trange(num_epochs):
            ...
            if epoch > 0:
                ...
                for bidx, bparts in enumerate(dataloader_train.get_iterator(batch_size=batch_size, device=torch.device("cpu"), shuffle=True, xrange=trange)):
                    ...
                    loss = diceloss(pred_bx1KxHxW, label_bxHxW) + diceloss_1(seg_pred_bx1xHxW + 0.5, cur_Y_bx1xHxW)
            ...
            if True:
                ...
                with torch.no_grad():
                    ...
                    predictions = torch.argmax(torch.cat(cls_predictions_bx1xK_s, dim=0).squeeze(1), dim=1).detach().cpu().numpy()
                    targets = torch.cat(Y_cls_bx1_s, dim=0).squeeze(1).detach().cpu().numpy().T
                    ...
                    print("lvis")
                    print(df[df['label'] == 'MEAN'])
                    with open('output_lvis_640_cls_implicit_backclass_1e4.txt', 'a', encoding='utf-8') as f:
                        print("lvis", file=f)
                        print(df[df['label'] == 'MEAN'], file=f)



        # freeze_parameters(efficientsam_ti_custom.mask_decoder)
        # unfreeze_parameters(efficientsam_ti_custom.mask_decoder.classify_model)
        ...
        #         {"params": efficientsam_ti_custom.mask_decoder.classify_model.parameters(), "lr": lr},
        #     ]
        # )
        ...
        #                 input_labels_bx1xN,
        #             )
        #             # seg_pred_bx1xHxW = sigmoid(output_masks_bx1x1xHxW[:, 0, :, :, :])
        #             seg_pred_bx1xHxW = nn.functional.sigmoid(output_masks_bx1x1xHxW[:, 0, :, :, :]).float()
        #             cur_Y_bx1xHxW = Y_bx1xHxW[:, :, :, :].to(device=device)
        ...
        #             print("lvis")
        #             print(df[df['label'] == 'MEAN'])
        #             with open('output_lvis_640_cls_implicit_backclass.txt', 'a', encoding='utf-8') as f:
        #                 print("lvis", file=f)
        #                 print(df[df['label'] == 'MEAN'], file=f)
