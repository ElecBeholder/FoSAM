...
from l_sam.forveated_sam.foveated_sam_implicit_backclass_cls_useimage_embed import build_foveated_sam
        ...
        for epoch in trange(num_epochs):
            ...
            if True:
                ...
                with torch.no_grad():
                    ...
                    with open('output_lvis_640_cls_implicit_backclass_1e-2_seperate_cls_useimage_embed_res18.txt', 'a', encoding='utf-8') as f:
        ...
        for epoch in trange(num_epochs):
            ...
            if epoch > 0:
                ...
                for bidx, bparts in enumerate(dataloader_train.get_iterator(batch_size=batch_size, device=torch.device("cpu"), shuffle=True, xrange=trange)):
                    ...
                    global_step = global_step + 1
                    writer.add_scalar('Loss_cls_train', diceloss(pred_bx1KxHxW, label_bxHxW).item(), global_step)
            ...
            if True:
                ...
                with torch.no_grad():
                    ...
                    for bidx, bparts in enumerate(dataloader_valid.get_iterator(batch_size=5, device=torch.device("cpu"), shuffle=True, xrange=range)):
                        ...
                        writer.add_scalar('Loss_cls_val', diceloss(pred_bx1KxHxW, label_bxHxW).item(), global_step)
                        # mcmiou = compute_multiclass_iou(pred_bxHxW.to(device=device), label_bxHxW.to(device=device), class_num, ignore_index=0)
                        mcmiou = compute_multiclass_iou(pred_bx1KxHxW, label_bxHxW)
