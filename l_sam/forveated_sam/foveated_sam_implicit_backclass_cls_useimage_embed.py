...
def get_custom_resnet(num_classes=51, in_channels=258):
    """
...
class FoveatedSam(nn.Module):
    ...
    def forward(
        ...
        mean = fusion_emd.mean(dim=(2, 3), keepdim=True)  # [B, C, H, W]
        std = fusion_emd.std(dim=(2, 3), keepdim=True) + 1e-6 
        
        fusion_emd = (fusion_emd - mean) / std
