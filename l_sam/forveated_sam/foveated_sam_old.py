...
def crop_and_resize(batched_images, masks, size=80):
    ...
    for i in range(B):
        ...
        # 计算边界框
        y_min = coords[:, 0].min()  # 所有 y 坐标的最小值
        x_min = coords[:, 1].min()  # 所有 x 坐标的最小值
        y_max = coords[:, 0].max()  # 所有 y 坐标的最大值
        x_max = coords[:, 1].max()  # 所有 x 坐标的最大值

        # 裁剪图像
        cropped = (image * torch.sigmoid(mask_resized.squeeze(1)))[:, y_min:y_max + 1, x_min:x_max + 1]  # (C, h, w)
        ...
        # try:
        #     save_image(padded.squeeze(0), f"view/{i}.png")
...
class FoveatedSam(nn.Module):
    ...
    def __init__(
        ...
        self.classify_model = MobileNetV2(in_channels=3, out_channels=class_num+1)
    ...
    def forward(
        ...
        # image_BxCxHxW = crop_and_resize(batched_images, masks.squeeze(1).squeeze(1), 160)
        ...
        #     masked_image = masked_images_BxCxHxW[i]  # (C, H, W)
            ...
            # try:
            #     save_image(masked_image.squeeze(0), f"view/{i}.png")
            # except Exception as err:
            #     print(traceback.format_exc())

        cls_pred = F.softmax(self.classify_model(masked_images_BxCxHxW), dim=1)
...
def build_foveated_sam(img_size, encoder_patch_embed_dim, encoder_num_heads, num_multimask_outputs=3, class_num=10, checkpoint_state_dict=None):
    ...
    if checkpoint_state_dict is not None:
        sam.apply(init_weights_random)
        sam.load_state_dict(checkpoint_state_dict, strict=False)
        sam.classify_model.apply(init_weights_random)
    return sam