...
from l_sam.forveated_sam.efficient_sam_encoder_saliency_joint_prune_learnable_downsample_multigaussian import ImageEncoderViT
...
def generate_attention_mask(images, gaze_coords, crop_ratio=0.5):
    B, C, H, W = images.shape
    
    # 初始化全零掩码
    mask = torch.zeros((B, 1, H, W), dtype=torch.float32, device=images.device)
    crop_size = int(min(H, W) * crop_ratio)  # 根据图像尺寸计算裁剪大小

    for i in range(B):
        # 获取 gaze 归一化坐标，并缩放到图像尺寸
        x_g, y_g = gaze_coords[i] # coord for 640*640
