...
from l_sam.forveated_sam.efficient_sam_encoder_saliency_joint_prune import ImageEncoderViT
...
def get_custom_resnet(num_classes=51, in_channels=3):
    """
    创建一个自定义的 ResNet18，用于输入通道数为 in_channels 的图像分类任务。
    最终输出维度为 num_classes。
    """
    # 1. 加载官方预训练的 ResNet18（会下载预训练权重，但仅适用于原始 3 通道；我们会修改第一层）
    resnet = models.resnet18(pretrained=False)
    
    # 2. 修改第一层的卷积层，使其适应自定义输入通道数（原先是 3 通道，这里改为 in_channels）
    #    参数含义： out_channels=64, kernel_size=7, stride=2, padding=3, bias=False
    ...
    # 3. 修改最后一层全连接层（fc），输出维度改为 num_classes
    #    对于 ResNet18，fc 的输入维度是 512
...
class DualResNet(nn.Module):
    def __init__(self, num_classes=51, 
                 base_model='resnet18', 
                 feat_dim=512):  # 对于resnet18/34，feat_dim=512；对于resnet50/101，feat_dim=2048
        super().__init__()
        ...
        # 修改conv1，使其输入通道=256, out_channels=64
        ...
        # ============ 分类层 ============
        # 两个分支分别输出 feat_dim 维，拼接后 2*feat_dim
    ...
    def forward(self, x1, x2):
        ...
        """
        # 分别通过两个 resnet
        feat1 = self.resnet1(x1)  # 形状 [B, feat_dim]
        feat2 = self.resnet2(x2)  # 形状 [B, feat_dim]

        # 拼接
        feats = torch.cat([feat1, feat2], dim=1)  # [B, 2*feat_dim]
...
# 双卷积模块：连续两个 3×3 卷积 + BatchNorm + ReLU（padding=1 保持尺寸）
...
class Up(nn.Module):
    ...
    def forward(self, x1, x2):
        x1 = self.up(x1)
        # 若尺寸不匹配则进行 padding（此处一般匹配）
        diffY = x2.size()[2] - x1.size()[2]
        diffX = x2.size()[3] - x1.size()[3]
        x1 = F.pad(x1, [diffX // 2, diffX - diffX // 2,
                        diffY // 2, diffY - diffY // 2])
        # 拼接后通道数为：encoder 层 + 上采样后的解码层
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x6 = self.down5(x5)    # B x 2048 x 20 x 20  (最底层)
        
        # 解码器，逐步上采样并融合对应的编码器特征
...
def generate_attention_mask(images, gaze_coords, crop_size=320):
    """
    生成一个与输入图像相同大小的 0-1 掩码，其中 gaze 位置的窗口设为 1，其余部分设为 0。

    Args:
        images (Tensor): 形状 [B, C, H, W] 的输入图像。
        gaze_coords (Tensor): 形状 [B, 2] 的 gaze 原始坐标 (x, y)。
        crop_size (int): 目标窗口的尺寸（默认 20x20）。

    Returns:
        mask (Tensor): 形状 [B, 1, H, W] 的 0-1 掩码，窗口区域为 1，其余为 0。
    """
    ...
    for i in range(B):
        ...
        mask = torch.nn.functional.adaptive_max_pool2d(mask, (40, 40)) 
...
def token_pruning(images, gaze_coords, crop_size=20):
    """
    从每张图片中裁剪出一个子图，使 gaze 尽量位于中心，并计算 gaze 的新坐标。

    Args:
        images (Tensor): 形状 [B, C, H, W] 的输入图像。
        gaze_coords (Tensor): 形状 [B, 2] 的 gaze 原始坐标 (x, y)。
        crop_size (int): 子图的目标尺寸（默认 20x20）。
    
    Returns:
        cropped_images (Tensor): 裁剪后的图像，形状 [B, C, crop_size, crop_size]。
        new_gaze_coords (Tensor): gaze 在裁剪后子图中的新坐标，形状 [B, 2]。
    """
    B, C, H, W = images.shape
...
class FoveatedSam(nn.Module):
    ...
    def __init__(
        ...
        self.saliency = UNet(n_channels=5, n_classes=1, bilinear=True)
    ...
    # @watch_time
    @torch.jit.export
    def get_image_embeddings(self, batched_images, s_bin_selected_BxHMxWMx1) -> torch.Tensor:
        """
        ...
        return self.image_encoder(batched_images, s_bin_selected_BxHMxWMx1)
    ...
    def forward(
        ...
        # fusion_crop = fuse_gaze_distance_encoder_output_advance(batched_images.cuda(), batched_points.squeeze(1).squeeze(1))

        # mean_global = torch.mean(fusion_crop, dim=(0, 2, 3))  # 对 B, H, W 维度取均值
        # std_global = torch.std(fusion_crop, dim=(0, 2, 3), unbiased=False)  # 对 B, H, W 维度取标准差
        ...
        # saliency_outcome_1 = self.saliency(fusion_crop).permute(0, 2, 3, 1)
        # B, HT, WT, D = saliency_outcome_1.shape
        # saliency_outcome_1 = saliency_outcome_1.view(B, -1)

        # temperature = 0.1
        # saliency_outcome_2 = torch.sigmoid((saliency_outcome_1) / temperature).view(B, HT, WT, D)
        # print(saliency_outcome_2.shape)
        # print(generate_attention_mask(batched_images, batched_points.squeeze(1).squeeze(1)).shape)

        # s_bin_selected_BxHMxWMx1 = generate_attention_mask(batched_images, batched_points.squeeze(1).squeeze(1)).view(B, HT, WT, D) - 0.5 + saliency_outcome_2
        # print(s_bin_selected_BxHMxWMx1.shape)

        s_bin_selected_BxHMxWMx1 = generate_attention_mask(batched_images, batched_points.squeeze(1).squeeze(1))




        image_embeddings = self.get_image_embeddings(batched_images, s_bin_selected_BxHMxWMx1)
        ...
        mean_global = torch.mean(fusion_emd, dim=(0, 2, 3))  # 对 B, H, W 维度取均值
        std_global = torch.std(fusion_emd, dim=(0, 2, 3), unbiased=False)  # 对 B, H, W 维度取标准差
        ...
        # image_BxCxHxW = crop_and_resize(batched_images, masks.squeeze(1).squeeze(1), 256)
        
        # mean_global = torch.mean(image_BxCxHxW, dim=(0, 2, 3))  # 对 B, H, W 维度取均值
        # std_global = torch.std(image_BxCxHxW, dim=(0, 2, 3), unbiased=False)  # 对 B, H, W 维度取标准差
        ...
        # # 对图像张量批量应用预处理
        # image_BxCxHxW = preprocess(image_BxCxHxW)

        # cls_pred = self.classifier(image_BxCxHxW)
