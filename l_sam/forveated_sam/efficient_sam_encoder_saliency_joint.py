...
class ImageEncoderViT(nn.Module):
    ...
    def forward(self, x_BxCxHxW: torch.Tensor, s_bin_selected_BxHMxWMx1: torch.Tensor = None) -> torch.Tensor:
        ...
        B, HM, WM, D = restore_x_merged_BxHMxWMxD.shape

        # 1. 构造形状为 (B, HM, WM, D, 2, 2) 的全 0 tensor
        expanded_tensor = torch.zeros(B, HM, WM, D, 2, 2,
                                    device=restore_x_merged_BxHMxWMxD.device,
                                    dtype=restore_x_merged_BxHMxWMxD.dtype)

        # 2. 将原始值放到 2x2 patch 的左上角位置
        expanded_tensor[..., 0, 0] = restore_x_merged_BxHMxWMxD

        # 3. 如果你需要通过 cross_fold 恢复为 (B, HT, WT, D)（其中 HT=HM*2, WT=WM*2），则调用：
        restore_x_merged_BxHTxWTxD = cross_fold(expanded_tensor)
