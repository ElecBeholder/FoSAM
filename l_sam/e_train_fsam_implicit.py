...
from l_sam.forveated_sam.foveated_sam_implicit import build_foveated_sam
...
if __name__ == '__main__':
    ...
    if True:
        ...
        for epoch in trange(num_epochs):
            ...
            if epoch > 0:
                ...
                for bidx, bparts in enumerate(dataloader_train.get_iterator(batch_size=batch_size, device=torch.device("cpu"), shuffle=True, xrange=trange)):
                    ...
                    pred_bx1KxHxW = merge_seg_cls(seg_pred_bx1xHxW, cls_pred_bxK)
                    label_bxHxW = merge_seg_label(cur_Y_bx1xHxW, Y_cls_b)


                    # loss = diceloss(seg_pred_bx1xHxW, cur_Y_bx1xHxW) + celoss(pred_bx1KxHxW, label_bxHxW)
                    loss = diceloss(seg_pred_bx1xHxW, cur_Y_bx1xHxW)
            ...
            if True:
                ...
                with torch.no_grad():
                    ...
                    for bidx, bparts in enumerate(dataloader_valid.get_iterator(batch_size=5, device=torch.device("cpu"), shuffle=True, xrange=range)):
                        ...
                        mcmiou = compute_multiclass_iou(pred_bxHxW.to(device=device), label_bxHxW.to(device=device), class_num + 1, ignore_index=0)
                    ...
                    print("lvis")
                    print(df[df['label'] == 'MEAN'])
                    with open('output_lvis_640_cls_implicit.txt', 'a', encoding='utf-8') as f:
                        print("lvis", file=f)
        ...
        #             print("lvis")
        #             print(df[df['label'] == 'MEAN'])
        #             with open('output_lvis_640_cls_adaptive.txt', 'a', encoding='utf-8') as f:
        #                 print("lvis", file=f)
        #                 print(df[df['label'] == 'MEAN'], file=f)
