    ...
    if True:
        ...
        for epoch in trange(num_epochs):
            ...
            if epoch > 0:
                ...
                for bidx, bparts in enumerate(dataloader_train.get_iterator(batch_size=batch_size, device=torch.device("cpu"), shuffle=True, xrange=range)):
            ...
            if True:
                ...
                with torch.no_grad():
                    ...
                    # 调用评估函数并打印结果
                    f1_per_class, accuracy_per_class, precision_per_class, recall_per_class = evaluate_classification_overall(
                        predictions, targets, class_num=class_num
                    )
                    ...
                    col2elems['f1_cls'].append(f1_per_class)

                    df = pd.DataFrame(col2elems)
                    print()
                    print(df[df['label'] == 'MEAN'])

                    scheduler.step(f1_per_class + mean_seg_fg_iou)

                    torch.save(efficientsam_ti_custom.state_dict(), os.path.join(dpath, 'checkpoints.pth'))
                    df.to_csv(os.path.join(dpath, 'evaluation.csv'), index=False)
                    save_json({'epoch': epoch}, os.path.join(dpath, 'info.json'))
