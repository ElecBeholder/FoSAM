...
if __name__ == '__main__':
    # old_state_dict = torch.load(os.path.join(preset.dpath_esam_weights, 'efficient_sam_vitt.singlemask.pt'))
    ...
    torch.save(old_state_dict, os.path.join(preset.dpath_esam_weights, 'efficient_sam_vits.singlemask.pt'))
        ...
        for epoch in trange(num_epochs):
            ...
            if epoch > 0:
                ...
                for bidx, bparts in enumerate(dataloader_train.get_iterator(batch_size=batch_size, device=torch.device("cpu"), shuffle=True, xrange=trange)):
                    ...
                    loss = BFLoss(sigmoid(predicted_logits_bx1x3xHxW[:,0,:,:,:]), Y_bx1xHxW[:, :, :, :].to(device=device))
