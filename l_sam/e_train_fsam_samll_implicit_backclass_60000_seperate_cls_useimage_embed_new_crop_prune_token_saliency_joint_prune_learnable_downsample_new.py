...
from l_sam.forveated_sam.foveated_sam_implicit_backclass_cls_useimage_embed_crop_prune_token_advance_res101_saliency_joint_prune_learnable_downsample_new import build_foveated_sam
...
writer = SummaryWriter(log_dir='/root/autodl-tmp/DynamicFocus_new/l_sam_experiment')
...
if __name__ == '__main__':
    ...
    old_state_dict = torch.load("/root/autodl-tmp/DynamicFocus_new/l_sam/efficient_sam_vits.singlemask.pt")['model']
        ...
        for epoch in trange(num_epochs):
            ...
            if epoch > 0:
                ...
                for bidx, bparts in enumerate(dataloader_train.get_iterator(batch_size=batch_size, device=torch.device("cpu"), shuffle=True, xrange=trange)):
                    for name, param in efficientsam_ti_custom.image_encoder.segmentation_module.net_compress.named_parameters():
                        param.requires_grad_(True)
            ...
            if True:
                ...
                with torch.no_grad():
                    ...
                    with open('learning_to_downsample.txt', 'a', encoding='utf-8') as f:
