...
if __name__ == '__main__':
    ...
    if True:
        ...
        for epoch in trange(num_epochs):
            ...
            if epoch > 0:
                ...
                for bidx, bparts in enumerate(dataloader_train.get_iterator(batch_size=batch_size, device=torch.device("cpu"), shuffle=True, xrange=trange)):
                    ...
                    loss = 10*diceloss(pred_bx1KxHxW, label_bxHxW) + diceloss_1(seg_pred_bx1xHxW + 0.5, cur_Y_bx1xHxW)
                    ...
                    writer.add_scalar('Loss_cls', 10*diceloss(pred_bx1KxHxW, label_bxHxW).item(), global_step)
            ...
            if True:
                ...
                with torch.no_grad():
                    ...
                    with open('output_lvis_640_cls_implicit_backclass_1e3_12000.txt', 'a', encoding='utf-8') as f:
        ...
        #             scheduler.step(mean_cls_foreground_miou)
        #             predictions = torch.argmax(torch.cat(cls_predictions_bx1xK_s, dim=0).squeeze(1), dim=1).detach()
        #             targets = torch.cat(Y_cls_bx1_s, dim=0).squeeze(1).detach().T
        #             # accuracy = np.sum(predictions == targets) / len(predictions)
        #             # print()
        #             # print(predictions.tolist())
        #             # print(targets.tolist())
