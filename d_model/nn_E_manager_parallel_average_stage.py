...
class ModelManager:
    ...
    def train(self, take_model: nn.Module, target_device: str, loss_fctn_seg: Optional[nn.modules.loss._Loss], loss_fctn_cls: Optional[nn.modules.loss._Loss], wloss: List, optimizer: torch.optim.Optimizer,
        ...
        for i, data in enumerate(datasetloader_train.get_iterator(batch_size=batch_size, device=target_device, shuffle=True)):
            ...
            if isinstance(check_model, SegerZoom) or isinstance(check_model, SegerZoomSeg):
    ...
    def plot_figure(self, datasetloader: CustomDataLoader, fname=f"plot_figure.png", rank=0):
        ...
        with torch.no_grad():
            if isinstance(self.model, SegerZoom):
                ...
                mgpu.y_pred_gs_bx1xHSxWS, mgpu.grid_pred_bxHSxWSx2, mgpu.dmap_pred_ds_bx1xHSxWS, mgpu.x_ds_rgbaf_bxC1xHSxWS, mgpu.x_gs_rgbaf_bxC1xHSxWS, mgpu.y_pred_bxK = check_model(mgpu.x_bx4xHxW, mgpu.x_bx2)
...
def get_sys_kwargs():
    ...
    parser.add_argument('--class_num', default=5, help='Class Number')
