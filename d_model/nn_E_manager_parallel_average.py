...
def main(rank, world_size):
    ...
    if Skwargs.ckpt_path is not None:
        ...
        checkpoint = torch.load(Skwargs.ckpt_path, map_location=f'cuda:{rank}')
        filtered_state_dict = {}
        keywords = ["gen_seg_0"] 

        # Filter the state_dict based on the keywords
        for k, v in checkpoint.items():
            if any(keyword in k for keyword in keywords):
                print(k)
                filtered_state_dict[k] = v
        nn_module.load_state_dict(filtered_state_dict, strict=False)
    ...
    save_per_N_epoch = 30
    print_per_N_epoch = 10
    init_lr = 0.005 if 'Seg' not in modelname else 0.005
    decay = 0.9
    wloss = [0, 2, 0.1]
