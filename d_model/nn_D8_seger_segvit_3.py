...
class SegerZoomSeg(nn.Module):
    ...
    def __init__(self,
                 ...
                 priori=True,
                 square_focus=True):
        super(SegerZoomSeg, self).__init__()
        ...
        if version == 4:
            config = SegformerConfig.from_pretrained("nvidia/segformer-b4-finetuned-cityscapes-1024-1024")
            config.num_labels = 1  
        else:
        ...
        self.transform_rgbaff = transforms.Compose([
            transforms.Normalize(mean=[0.485, 0.456, 0.406, 0.55, 0.00097], std=[0.229, 0.224, 0.225, 0.20, 0.0312362]) 
        ])
    ...
    def forward(self, x_BxCxHxW: torch.Tensor, x_Bx2: torch.Tensor):
        ...
        if self.priori:
            hidx_B = torch.clip(torch.round(x_Bx2[:, 0] * (HD - 1)), 0, WD - 1)
            widx_B = torch.clip(torch.round(x_Bx2[:, 1] * (WD - 1)), 0, WD - 1)
        ...
        # x_gs_rgbaff_BxC2xHSxWS[:, :-2, :, :] = self.ave_pool(x_BxCxHxW)
        ...
        y_pred_BxK = self.gen_cls(self.transform_rgbaff(x_gs_rgbaff_BxC2xHSxWS[:, [0, 1, 2, 3, 4]])*y_pred_gs_Bx1xHSxWS)
