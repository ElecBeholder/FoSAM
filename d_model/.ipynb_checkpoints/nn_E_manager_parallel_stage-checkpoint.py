...
from d_model.nn_C1_mobilenetv2 import MobileNetV2, CustomMobileNetV2
...
from d_model.nn_D3_seger_average_3 import SegerAverage
...
from d_model.nn_D9_seger_deeplab_3 import SegerZoomDeep
from d_model.nn_D7_serger_zoom_sam import SegerZoomSAM
# from d_model.nn_D10_seger_psp_3 import SegerZoomPSP
from d_model.nn_D10_psp_woprior import SegerZoomPSP
from d_model.nn_D11_seger_hr_3 import SegerZoomHR
...
from d_model.nn_A3_metrics import evaluate_segmentation, evaluate_classification, calc_cfmtx
...
class ModelManager:
    ...
    def train(self, take_model: nn.Module, target_device: str, loss_fctn_seg: Optional[nn.modules.loss._Loss], loss_fctn_cls: Optional[nn.modules.loss._Loss], wloss: List, optimizer: torch.optim.Optimizer,
        ...
        for i, data in enumerate(datasetloader_train.get_iterator(batch_size=batch_size, device=target_device, shuffle=True)):    
            ...
            if isinstance(check_model, SegerZoom) or isinstance(check_model, SegerZoomSeg) or isinstance(check_model, SegerZoomPSP) or isinstance(check_model, SegerZoomDeep) or isinstance(check_model, SegerZoomHR):
                y_pred_gs_bx1xHSxWS, grid_pred_bxHSxWSx2, dmap_pred_ds_bx1xHDxWD, x_ds_rgbaff_bxC1xHDxWD,x_gs_rgbaff_bxC1xHSxWS, y_pred_bxK = take_model(x_bx4xHxW, x_bx2)

                y_real_gs_bx1xHSxWS = check_model.downsample_y_gridsp_real_Bx1xHSxWS(y_bx1xHxW, grid_pred_bxHSxWSx2) # 
                y_real_ds_bx1xHDxWD = check_model.downsample_y_avepool_real_Bx1xHDxWD(y_bx1xHxW)

                w1, w2, w3 = wloss
                tensor_train_loss = (
                        w1 * loss_fctn_seg(dmap_pred_ds_bx1xHDxWD, y_real_ds_bx1xHDxWD) +
                ...
                tensor_train_loss = loss_fctn_seg(y_pred_gs_bx1xHSxWS, y_real_ds_bx1xHSxWS) + loss_fctn_cls(y_pred_bxK, y_bx1)
    ...
    def predict(self, *args):
        ...
        mgpu.x_bx4xHxW, mgpu.x_bx2, mgpu.bx1xHxW = args
        ...
        with torch.no_grad():
            if isinstance(check_model, SegerZoom) or isinstance(check_model, SegerZoomDeep) or isinstance(check_model, SegerZoomPSP) or isinstance(check_model, SegerZoomSeg) or isinstance(check_model, SegerZoomHR):

                y_pred_gs_bx1xHSxWS, grid_pred_bxHSxWSx2, dmap_pred_ds_bx1xHDxWD, x_ds_rgbaff_bxC1xHDxWD, x_gs_rgbaff_bxC1xHSxWS, y_pred_bxK = take_model(mgpu.x_bx4xHxW, mgpu.x_bx2)

                y_pred_bx1xHxW = check_model.output_y_pred_Bx1xHxW(y_pred_gs_bx1xHSxWS, grid_pred_bxHSxWSx2, mgpu.x_bx4xHxW)
                y_real_gs_bx1xHSxWS = check_model.downsample_y_gridsp_real_Bx1xHSxWS(mgpu.y_bx1xHxW, grid_pred_bxHSxWSx2)


            elif isinstance(check_model, SegerAverage) or isinstance(check_model, SegerUniform):
                y_pred_gs_bx1xHSxWS, x_ds_rgbaf_bxC1xHSxWS, y_pred_bxK = take_model(mgpu.x_bx4xHxW, mgpu.x_bx2)
                y_pred_bx1xHxW = check_model.output_y_pred_Bx1xHxW(y_pred_gs_bx1xHSxWS)

                y_real_gs_bx1xHSxWS = check_model.downsample_y_maxpool_real_Bx1xHSxWS(mgpu.y_bx1xHxW)


        return y_pred_bx1xHxW, y_pred_bxK, y_pred_gs_bx1xHSxWS, y_real_gs_bx1xHSxWS
    ...
    def get_metrics(self, class_num, datasetloader: CustomDataLoader, namekeys, dataset_partition, batch_size=5, show=False, save=False,
        ...
        MODE_miou_per_obj = 'miou_per_obj'
        MODE_miou_per_cls = 'miou_per_cls'

        mode = MODE_miou_per_cls

        print(f"use metrics {mode}")

        if mode == MODE_miou_per_obj:
            ...
            for i, data in enumerate(datasetloader.get_iterator(batch_size=batch_size, device=target_device, shuffle=False, xrange=xrange)):
                ...
                mgpu.y_pred_bx1xHxW, mgpu.y_pred_bxK, mgpu.y_pred_gs_bx1xHSxWS, mgpu.y_real_gs_bx1xHSxWS  = self.predict(mgpu.x_bx4xHxW, mgpu.x_bx2, mgpu.y_bx1xHxW)

                del mgpu.x_bx4xHxW
                del mgpu.x_bx2

                seg_iou_B, seg_f1_B, seg_accuracy_B, seg_precision_B, seg_recall_B = evaluate_segmentation(mgpu.y_pred_bx1xHxW, mgpu.y_bx1xHxW)
                seg_iou_b, _, _, _, _ = evaluate_segmentation(mgpu.y_pred_gs_bx1xHSxWS, mgpu.y_real_gs_bx1xHSxWS)
            ...
            for bidx, namekey in enumerate(namekeys):
                ...
                labelkey2kidx[labelkey] = convert_index(k)
            ...
            for labelkey, bidxs in labelkey2bidxs.items():
                ...
                seg_iou_deformed = np.mean(np.array(seg_iou_s1)[bidxs])
                ...
                col2elems['seg_iou_deformed'].append(seg_iou_deformed)
            ...
            mean_seg_iou_deformed= np.array(col2elems['seg_iou_deformed']).mean()
            ...
            col2elems['seg_iou_deformed'].append(mean_seg_iou_deformed)
        ...
        elif mode == MODE_miou_per_cls:

            TP_b_s, TN_b_s, FP_b_s, FN_b_s = [], [], [], []
            TP_b_s1, TN_b_s1, FP_b_s1, FN_b_s1 = [], [], [], []
            ...
            for i, data in enumerate(datasetloader.get_iterator(batch_size=batch_size, device=target_device, shuffle=False, xrange=xrange)):
                ...
                TP_b, TN_b, FP_b, FN_b = calc_cfmtx(mgpu.y_pred_bx1xHxW, mgpu.y_bx1xHxW)
                TP_b_1, TN_b_1, FP_b_1, FN_b_1 = calc_cfmtx(mgpu.y_pred_gs_bx1xHSxWS, mgpu.y_real_gs_bx1xHSxWS)
                ...
                TP_b_s1.extend(TP_b_1)
                TN_b_s1.extend(TN_b_1)
                FP_b_s1.extend(FP_b_1)
                FN_b_s1.extend(FN_b_1)
            ...
            for labelkey, bidxs in labelkey2bidxs.items():
                total_TP = np.sum(np.array(TP_b_s)[bidxs])
                total_FP = np.sum(np.array(FP_b_s)[bidxs])
                total_FN = np.sum(np.array(FN_b_s)[bidxs])
                total_TN = np.sum(np.array(TN_b_s)[bidxs])

                eps = 1e-7
                seg_iou = total_TP / (total_TP + total_FP + total_FN + eps)
                seg_iou_deformed = np.sum(np.array(TP_b_s1)[bidxs]) / (np.sum(np.array(TP_b_s1)[bidxs]) + np.sum(np.array(FP_b_s1)[bidxs]) + np.sum(np.array(FN_b_s1)[bidxs]) + eps)
                seg_precision = total_TP / (total_TP + total_FP + eps)
                seg_recall = total_TP / (total_TP + total_FN + eps)
                seg_accuracy = (total_TP + total_TN) / (total_TP + total_TN + total_FP + total_FN + eps)
                seg_f1 = 2 * (seg_precision * seg_recall) / (seg_precision + seg_recall + eps)
        ...
        if save:
            ...
            fpath_metrics = os.path.join(preset.dpath_training_records, f"{self.model_name}", f'{dataset_partition}_{rank}_{stage}.metrics.csv')
        ...
        mgpu.delete_all()
        mgpu.gc()
        return mean_seg_iou, mean_seg_iou_deformed, mean_seg_f1, mean_seg_accuracy, mean_seg_precision, mean_seg_recall, mean_cls_f1, mean_cls_accuracy, mean_cls_precision, mean_cls_recall
    ...
    def plot_figure(self, datasetloader: CustomDataLoader, fname=f"plot_figure.png", rank=0):
        ...
        with torch.no_grad():
            if isinstance(check_model, SegerZoom) or isinstance(check_model, SegerZoomDeep) or isinstance(check_model, SegerZoomSeg) or isinstance(check_model, SegerZoomDeep) :
                ...
                mgpu.y_pred_gs_bx1xHSxWS, mgpu.grid_pred_bxHSxWSx2, mgpu.dmap_pred_ds_bx1xHDxWD, mgpu.x_ds_rgbaff_bxC1xHDxWD, mgpu.x_gs_rgbaff_bxC1xHSxWS, mgpu.y_pred_bxK = check_model(mgpu.x_bx4xHxW, mgpu.x_bx2)
                ...
                for b in range(B):
                    imgs.extend([
                        ...
                        mgpu.y_pred_gs_bx1xHSxWS[b],
                        mgpu.y_real_gs_bx1xHSxWS[b]
...
def get_sys_kwargs():
    ...
    parser.add_argument('--Kblur', type=int, default=48, required=False, help=" (e.g., 2 4 8 ,16,32).")
    parser.add_argument('--Kprio', type=int, default=48, required=False, help=" (e.g., 2 4 8 ,16,32).")
    parser.add_argument('--Kgrid', type=int, default=48, required=False, help=" (e.g., 2 4 8 ,16,32).")
    ...
    parser.add_argument('--segformer_depth', type=int, default=4, required=False, help='Version of Segformer')
    parser.add_argument('--seg_module', type=str, choices=['deeplab', 'segformer_4', 'segformer_5', 'pspnet', 'hrnet'], 
                    required=False, help='Segmentation module to use')
...
def main(rank, world_size):
    ...
    base_module = UNet6
    classify_module = CustomMobileNetV2
    ...
    if module_name == 'SegerZoom':
        ...
        nn_module = SegerZoomSeg(base_module_deformation=base_module_deformation,
                        ...
                        kernel_priori=Skwargs.Kprio + 1,
                        square_focus=Skwargs.param_square_focus,
                        version=Skwargs.segformer_depth
                        )
    elif module_name=='SegerZoomPSP':
        nn_module = SegerZoomPSP(base_module_deformation=base_module_deformation,
                        base_module=base_module,
                        classify_module=classify_module,
                        in_channels=in_channels,
                        out_channels=out_channels,
    ...
    elif module_name=='SegerZoomHR':
        nn_module = SegerZoomHR(base_module_deformation=base_module_deformation,
                        base_module=base_module,
                        classify_module=classify_module,
                        in_channels=in_channels,
                        out_channels=out_channels,
        ...
        nn_module = SegerAverage(base_module=base_module, 
                                classify_module=classify_module,
                                in_channels=in_channels,
                                 out_channels=out_channels,
                                 downsample_factor=Skwargs.downsample_factor,
    ...
    """
    if Skwargs.ckpt_path is None and 'Average' not in modelname:
        finetune_module_name = 'seg_0' 
        for name, param in nn_module.named_parameters():
            if finetune_module_name in name:
                param.requires_grad = True
            else:
                param.requires_grad = False

        mm = ModelManager(nn_module, modelname, target_device, refresh=True)
        ...
        optimizer = torch.optim.NAdam(filter(lambda p: p.requires_grad, mm.model.parameters()), lr=init_lr, weight_decay=1e-5)
        lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=decay, patience=10)
        earlystop = EarlyStopMin()
        save_trigger = False
        mgpu = RAM()

        batch_size_train = Skwargs.batch_size_train//world_size//2 # 250 per batch
        batch_size_val = Skwargs.batch_size_valid//world_size//2 # 100 per batch
        ...
        if Skwargs.train:
            for ep in trange(epoch):
                ...
                if (ep+1) % plot_per_N_epoch == 0:
                    mm.plot_figure(datasetloader_train, fname=f"ep{ep}_stage1_train_plot.png", rank=rank)
                    mm.plot_figure(datasetloader_valid, fname=f"ep{ep}_stage1_valid_plot.png", rank=rank)

                if (ep+1) % 50 == 0:
                    if rank==0:
                        mm.save_model_state(mm.fpath_work_pt, msg=f'Best Epoch : {ep} loss={cur_loss_train:.6f}')

                if cur_loss_train<cur_loss:
                    cur_loss = cur_loss_train
                    cur_early = 0
                    if rank==0 and (ep+1) % 10==0:
                        mm.save_model_state(mm.fpath_work_pt, msg=f'Best Epoch : {ep} loss={cur_loss_train:.6f}')
                else:
                    cur_early += 1
                    if cur_early>patience:
                        break
        ...
        """
        -------------------------------------------------------------------------------
        Finetune Process of Seg
        """

        finetune_module_name = 'seg' 

        for name, param in nn_module.named_parameters():
            if finetune_module_name in name and 'seg_0' not in name:
                param.requires_grad = True
            else:
                param.requires_grad = False
        ...
        save_per_N_epoch = 30
        print_per_N_epoch = 10
        init_lr = 0.001 if 'Seg' not in modelname else 0.0001
        decay = 0.9
        wloss = [0, 2, 0.]
        ...
        factor = 2
        if 'SegerZoomSeg' in module_name or 'SegerZoomHR' in module_name:
            factor = 4
        batch_size_train = Skwargs.batch_size_train // factor // world_size 
        batch_size_val = Skwargs.batch_size_valid // factor // world_size 
        ...
        if Skwargs.train:
            for ep in trange(epoch):
                ...
                if (ep+1) % plot_per_N_epoch == 0:
                    ...
                    #                     show=True, save=False, rank=rank, world_size=world_size, stage=2)
                ...
                if save_trigger:
                    ...
                    mean_seg_iou, mean_seg_iou_deformed, mean_seg_f1, mean_seg_accuracy, mean_seg_precision, mean_seg_recall, mean_cls_f1, mean_cls_accuracy, mean_cls_precision, mean_cls_recall = res_bind
                    mean_score = 0.5 * mean_seg_f1 + 0.5 * mean_cls_f1
                    if earlystop.check(mean_seg_iou):
                        if rank==0:
                            mm.save_model_state(mm.fpath_work_pt, msg=f'Best Epoch : {ep} loss={cur_loss_train:.6f}')
                            mm.get_metrics(class_num=class_num, datasetloader=datasetloader_train, namekeys=datasetloader_train.dataset.get_namekeys(), dataset_partition=datasetloader_train.dataset.dataset_partition, batch_size=batch_size_val,
                                        show=True, save=True, rank=rank, world_size=world_size, stage=2)
                            mm.get_metrics(class_num=class_num, datasetloader=datasetloader_valid, namekeys=datasetloader_valid.dataset.get_namekeys(), dataset_partition=datasetloader_valid.dataset.dataset_partition, batch_size=batch_size_val,
                                    show=True, save=True, rank=rank, world_size=world_size, stage=2)
                torch.distributed.barrier()
    ...
    batch_size_train = Skwargs.batch_size_train // 2 // world_size 
    batch_size_val = Skwargs.batch_size_valid // 2 // world_size 
    ...
    if Skwargs.train:
        for ep in trange(epoch):
            ...
            #     mm.plot_figure(datasetloader_valid, fname=f"ep{ep}_stage3_valid_plot.png", rank=rank)
                ...
                # mm.get_metrics(class_num=class_num, datasetloader=datasetloader_train, namekeys=datasetloader_train.dataset.get_namekeys(), dataset_partition=datasetloader_train.dataset.dataset_partition, batch_size=batch_size_val,
                #             show=True, save=False, rank=rank, world_size=world_size,stage=3)
                # mm.get_metrics(class_num=class_num, datasetloader=datasetloader_valid, namekeys=datasetloader_valid.dataset.get_namekeys(), dataset_partition=datasetloader_valid.dataset.dataset_partition, batch_size=batch_size_val,
                #         show=True, save=False, rank=rank, world_size=world_size, stage=3)
            ...
            if save_trigger:
                ...
                mean_seg_iou, mean_seg_iou_deformed, mean_seg_f1, mean_seg_accuracy, mean_seg_precision, mean_seg_recall, mean_cls_f1, mean_cls_accuracy, mean_cls_precision, mean_cls_recall = res_bind
                mean_score = 0.5 * mean_seg_f1 + 0.5 * mean_cls_f1
                if earlystop.check(mean_cls_f1):
                    if rank==0:
                        mm.save_model_state(mm.fpath_work_pt, msg=f'Best Epoch : {ep} loss={cur_loss_train:.6f}')
                    mm.get_metrics(class_num=class_num, datasetloader=datasetloader_train, namekeys=datasetloader_train.dataset.get_namekeys(), dataset_partition=datasetloader_train.dataset.dataset_partition, batch_size=batch_size_val,
