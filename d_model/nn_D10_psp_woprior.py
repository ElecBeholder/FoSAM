...
class SegerZoomPSP(nn.Module):
    ...
    def __init__(self,
                 ...
                 priori=True,
                 square_focus=True):
        super(SegerZoomPSP, self).__init__()
        ...
        self.gen_seg_0 = base_module_deformation(in_channels=in_channels, out_channels=out_channels, base_channels=cbase_seg0, layer_num=nlayer_seg0)

        self.gen_seg = CustomPSPNet(input_channels = in_channels)
        self.gen_cls = classify_module(in_channels=in_channels, out_channels=class_num)
    ...
    def forward(self, x_BxCxHxW: torch.Tensor, x_Bx2: torch.Tensor):
        ...
        # assign RGB color map
        x_gs_rgbaff_BxC2xHSxWS= F.grid_sample(x_BxCxHxW, grid_pred_BxHSxWSx2, mode='bilinear', align_corners=True)
  
        out1_Bx1xHSxWS = self.gen_seg(self.transform_rgbaff(x_gs_rgbaff_BxC2xHSxWS))

        y_pred_gs_Bx1xHSxWS = torch.sigmoid(out1_Bx1xHSxWS)

        y_pred_BxK = self.gen_cls(self.transform_rgbaff(x_gs_rgbaff_BxC2xHSxWS[:, [0, 1, 2]])*y_pred_gs_Bx1xHSxWS)
