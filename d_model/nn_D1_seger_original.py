...
class SegerZoomDeep(nn.Module):
    ...
    def __init__(self,
        ...
        self.gen_seg_0 = base_module_deformation(in_channels=in_channels, out_channels=1, base_channels=cbase_seg0, layer_num=nlayer_seg0)

        self.gen_seg = UNet6(in_channels=in_channels, out_channels=class_num, base_channels=cbase_seg, layer_num=nlayer_seg)
    ...
    def forward(self, x_BxCxHxW: torch.Tensor):
        ...
        x_ds_rgbaff_BxC2xHDxWD = torch.zeros(B, C, HD, WD).to(dtype=torch.float32, device=x_BxCxHxW.device)

        x_ds_rgbaff_BxC2xHDxWD = self.downsample_x_avepool_real_BxCxHDxWD(x_BxCxHxW)

        out0_Bx1xHDxWD = self.gen_seg_0(self.transform_rgbaff(x_ds_rgbaff_BxC2xHDxWD)) # batch,1,H,D

        dmap_pred_ds_Bx1xHDxWD = torch.sigmoid(out0_Bx1xHDxWD, dim=1) #batch,H,D

        # dmap_pred_ds_Bx1xHDxWD = dmap_pred_ds_BxHDxWD.unsqueeze(1) 
        ...
        x_gs_rgbaff_BxC2xHSxWS = torch.zeros(B, C, HS, WS).to(dtype=torch.float32, device=x_BxCxHxW.device)
        x_gs_rgbaff_BxC2xHSxWS = F.interpolate(x_BxCxHxW, grid_pred_BxHSxWSx2, mode='bilinear', align_corners=True)

        out1_Bx1xHSxWS = self.gen_seg(self.transform_rgbaff(x_gs_rgbaff_BxC2xHSxWS))

        y_pred_gs_Bx1xHSxWS = out1_Bx1xHSxWS

        return y_pred_gs_Bx1xHSxWS, grid_pred_BxHSxWSx2, out0_Bx1xHDxWD, x_ds_rgbaff_BxC2xHDxWD, x_gs_rgbaff_BxC2xHSxWS
