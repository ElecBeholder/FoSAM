...
# from d_model.nn_B9_EfficientNet import EfficientNetS, EfficientNetL

# from d_model.nn_B5_Unet8 import UNet8
# from d_model.nn_B5_Unet7 import UNet7
...
from d_model.nn_B7_SegNet import SegNet
...
from d_model.nn_A2_loss import BCOSIMLoss, BMSELoss, ABCELoss
from d_model.nn_A3_metrics import calc_cfmtx, evaluate_classification, evaluate_segmentation
...
def threshold_map(tensor, threshold=0.5):
    """
    将张量中大于给定阈值的部分映射为 1，其余部分保持不变。

    参数:
    - tensor (torch.Tensor): 输入的张量。
    - threshold (float): 用于二值化的阈值，默认为 0.5。

    返回:
    - torch.Tensor: 应用阈值后的张量。
    """
    # 使用布尔掩码将大于 threshold 的位置设为 1
...
class ModelManager:
    ...
    def train(self, take_model: nn.Module, target_device: str, loss_fctn_seg: Optional[nn.modules.loss._Loss], loss_fctn_cls: Optional[nn.modules.loss._Loss], wloss: List, optimizer: torch.optim.Optimizer,
        ...
        for i, data in enumerate(datasetloader_train.get_iterator(batch_size=batch_size, device=target_device, shuffle=True)):
            ...
            if isinstance(check_model, SegerZoom):
                ...
                w0, w1, w2 = wloss
                final_train_loss = w0 * ls0 + w1 * ls1 + w2 * torch.log(ls2)
                ...
                losses_2.append(ls2)
                w0, w1, w2 = wloss
                final_train_loss = w1 * ls1 + w2 * torch.log(ls2)
                final_train_loss.backward()

            mgpu.delete_all()
        mgpu.gc()

        total_lossitem_0 = (sum([ls * b for ls, b in zip(losses_0, bs)]) / sum(bs)).item() if len(losses_0) > 0 else 0.
        total_lossitem_1 = (sum([ls * b for ls, b in zip(losses_1, bs)]) / sum(bs)).item() if len(losses_1) > 0 else 0.
        total_lossitem_2 = (sum([ls * b for ls, b in zip(losses_2, bs)]) / sum(bs)).item() if len(losses_2) > 0 else 0.
        ...
        print()
        w0, w1, w2 = wloss
        print(f"{str(w0).zfill(5)} x {np.exp(total_lossitem_0):6f}")
        print(f"{str(w1).zfill(5)} x {np.exp(total_lossitem_1):6f}")
    ...
    def predict(self, *args):
        ...
        with torch.no_grad():
            if isinstance(check_model, SegerZoom):
                ...
                mgpu.y_pred_bx1xHxW = check_model.output_y_pred_Bx1xHxW(mgpu.y_pred_gs_bx1xHSxWS, mgpu.grid_pred_bxHSxWSx2)

                del mgpu.y_pred_gs_bx1xHSxWS
                del mgpu.grid_pred_bxHSxWSx2
            elif isinstance(check_model, SegerAverage) or isinstance(check_model, SegerUniform):
                mgpu.y_pred_gs_bx1xHSxWS, mgpu.x_ds_rgbaff_bxC1xHSxWS, mgpu.y_pred_bxK = take_model(mgpu.x_bx4xHxW, mgpu.x_bx2)
                del mgpu.x_ds_rgbaff_bxC1xHSxWS
    ...
    def get_metrics(self, class_num, datasetloader: CustomDataLoader, namekeys, dataset_partition, batch_size=5, xrange=range, fname=None):
        ...
        if mode == MODE_miou_per_obj:
            ...
            for i, data in enumerate(datasetloader.get_iterator(batch_size=batch_size, device=target_device, shuffle=False, xrange=xrange)):
                ...
                TP_b_s.extend(TP_b.tolist())
                TN_b_s.extend(TN_b.tolist())
                FP_b_s.extend(FP_b.tolist())
                FN_b_s.extend(FN_b.tolist())
            ...
            if isinstance(datasetloader.dataset, DatasetLVIS):
                for bidx, namekey in enumerate(namekeys):
                    ...
                    if labelkey not in labelkey2bidxs:
                        labelkey2bidxs[labelkey] = []
                    labelkey2bidxs[labelkey].append(bidx)
            elif isinstance(datasetloader.dataset, DatasetCityScapes):
        ...
        if fname:
            ...
            fpath_metrics = os.path.join(self.fpath_training_records_modelname, fname)
            print(df)
    ...
    def plot_figure(self, datasetloader: CustomDataLoader, fname=f"plot_figure.png"):
        ...
        with torch.no_grad():
            if isinstance(self.model, SegerZoom):
                ...
                mgpu.y_pred_bx1xHxW = self.model.output_y_pred_Bx1xHxW(mgpu.y_pred_gs_bx1xHSxWS, mgpu.grid_pred_bxHSxWSx2)
                ...
                for b in range(B):
                    imgs.extend([
                        ...
                        threshold_map(mgpu.y_pred_bx1xHxW[b]),
                        mgpu.y_bx1xHxW[b]
                    ...
                    titles.extend([
                        ...
                        f"sgmt_grid_rgba_trgt_4xHSxWS_{b}",
            ...
            if isinstance(self.model, SegerAverage) or isinstance(self.model, SegerUniform):
                rows = 5
                mgpu.x_bx4xHxW, mgpu.x_bx2, mgpu.y_bx1xHxW, mgpu.y_bx1 = next(iter(datasetloader.get_iterator(batch_size=rows, device=target_device, shuffle=True)))
                ...
                mgpu.y_real_ds_bx1xHSxWS = self.model.downsample_y_maxpool_real_Bx1xHSxWS(mgpu.y_bx1xHxW)

                mgpu.y_pred_ds_bx1xHSxWS, mgpu.x_ds_rgbaff_bxC1xHSxWS, mgpu.y_pred_bxK = self.model(mgpu.x_bx4xHxW, mgpu.x_bx2)

                mgpu.y_pred_bx1xHxW = self.model.output_y_pred_Bx1xHxW(mgpu.y_pred_ds_bx1xHSxWS)
                ...
                for b in range(B):
                    imgs.extend([
                        ...
                        mgpu.x_ds_rgbaff_bxC1xHSxWS[b, :3],
                        torch.cat([mgpu.x_ds_rgbaff_bxC1xHSxWS[b, :3], mgpu.y_pred_ds_bx1xHSxWS[b]]),
                        torch.cat([mgpu.x_ds_rgbaff_bxC1xHSxWS[b, :3], mgpu.y_real_ds_bx1xHSxWS[b]]),
                    ...
                    titles.extend([
                        f"x_bx3xHxW{b}",
                        f"x_bx3xHSxWS{b}",
                        f"sgmt_ds_rgba_pred_4xHSxWS_{b}",
                        f"sgmt_ds_rgba_trgt_4xHSxWS_{b}",

                        f"fina_pred_back_rgba_1xHxW_{b}",
                        f"fina_real_back_rgba_1xHxW_{b}"
                    ])

                plt_multi_imgshow(imgs, titles, row_col=(B, 6))
                fpath_plot = os.path.join(self.fpath_training_records_modelname, fname)
                plt.savefig(fpath_plot)
                plt.close('all')
                print(f"save to {fpath_plot}")
...
def get_sys_kwargs():
    ...
    parser.add_argument('--Kblur', type=int, default=16, required=False, help=" (e.g., 2 4 8 ,16,32).")
    parser.add_argument('--Kprio', type=int, default=16, required=False, help=" (e.g., 2 4 8 ,16,32).")
    parser.add_argument('--Kgrid', type=int, default=16, required=False, help=" (e.g., 2 4 8 ,16,32).")
...
if __name__ == '__main__':
    ...
    base_module_deformation = EfficientNetS
    base_module = EfficientNetL
    ...
    if Skwargs.dataset_class == 'DatasetLVIS':
        ...
        datasetloader_train = DataLoaderCityScapesRGBLabelMask(DatasetCityScapesRGBLabelMask(sp_default, dataset_partition=dp_train))
        datasetloader_valid = DataLoaderCityScapesRGBLabelMask(DatasetCityScapesRGBLabelMask(sp_default, dataset_partition=dp_valid))

    class_num = datasetloader_train.dataset.K
    cropH = datasetloader_train.dataset.cropH
    cropW = datasetloader_train.dataset.cropW

    B = len(datasetloader_train.dataset)

    module_name, lossname = Skwargs.Module_Loss.split('_')

    nn_loss_fctn_seg = ABCELoss()
    ...
    if module_name == 'SegerZoom':
        ...
        nn_module = SegerUniform(base_module=base_module,
                                 classify_module=classify_module,
                                 in_channels=in_channels,
                                 out_channels=out_channels,
                                 class_num=class_num,
    ...
    epoch = 500
    trycheck_per_N_epoch = 10
    forcecheck_per_N_epoch = 50

    if preset.pc_name == preset.PC_NAME_server_H100 or preset.pc_name == preset.PC_NAME_server_4090:
        epoch = 5000
        trycheck_per_N_epoch = 10
        forcecheck_per_N_epoch = 100
        ...
        for ep in trange(epoch):
            ...
            if ep % forcecheck_per_N_epoch == 0:
                ...
                mm.get_metrics(class_num=class_num, datasetloader=datasetloader_train, namekeys=datasetloader_train.dataset.get_namekeys(), dataset_partition=datasetloader_train.dataset.dataset_partition, batch_size=batch_size_val,
                               fname=f'ep{ep}.train.metrics.csv')
                mm.get_metrics(class_num=class_num, datasetloader=datasetloader_valid, namekeys=datasetloader_valid.dataset.get_namekeys(), dataset_partition=datasetloader_valid.dataset.dataset_partition, batch_size=batch_size_val,
                               fname=f'ep{ep}.valid.metrics.csv')

            if ep >= 0 and save_trigger:
                ...
                res_bind = mm.get_metrics(class_num=class_num, datasetloader=datasetloader_valid,
                                          ...
                                          batch_size=batch_size_val, fname=None)
                ...
                if earlystop.check(mean_seg_iou):
                    ...
                    mm.get_metrics(class_num=class_num, datasetloader=datasetloader_train, namekeys=datasetloader_train.dataset.get_namekeys(), dataset_partition=datasetloader_train.dataset.dataset_partition, batch_size=batch_size_val,
                                   fname=f'best.train.metrics.csv')
                    mm.get_metrics(class_num=class_num, datasetloader=datasetloader_valid, namekeys=datasetloader_valid.dataset.get_namekeys(), dataset_partition=datasetloader_valid.dataset.dataset_partition, batch_size=batch_size_val,
                                   fname=f'best.valid.metrics.csv')
                    mm.plot_figure(datasetloader_train, fname=f"best.train.plot.png")
                    mm.plot_figure(datasetloader_valid, fname=f"best.valid.plot.png")
...
CUDA_VISIBLE_DEVICES=1 python d_model/nn_E_manager.py --train --metrics --dataset_class DatasetCityScapes_rgblabel_mask --Module_Loss SegerZoom_BMSELoss --dataset_marker_train sp0 --dataset_marker_valid sp0
CUDA_VISIBLE_DEVICES=1 python d_model/nn_E_manager.py --train --metrics --dataset_class DatasetCityScapes --Module_Loss SegerZoom_BMSELoss --dataset_marker_train sp60 --dataset_marker_valid sp10

CUDA_VISIBLE_DEVICES=1 python d_model/nn_E_manager.py --train --metrics --dataset_class DatasetCityScapes --Module_Loss SegerAverage_BMSELoss --dataset_marker_train sp60 --dataset_marker_valid sp10



CUDA_VISIBLE_DEVICES=1 python d_model/nn_E_manager.py --train --metrics --dataset_class DatasetLVIS --Module_Loss SegerZoom_BMSELoss --dataset_marker_train sp100 --dataset_marker_valid sp20
