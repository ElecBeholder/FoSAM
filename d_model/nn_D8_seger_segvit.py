...
class SegerZoomSeg(nn.Module):
    ...
    def __init__(self,
        ...
        self.gen_cls = classify_module(in_channels=in_channels + 2, num_classes=class_num)
    ...
    def forward(self, x_BxCxHxW: torch.Tensor, x_Bx2: torch.Tensor):
        ...
        if self.in_channels == 4:
            dmap_pred_ds_Bx1xHDxWD = dmap_pred_ds_Bx1xHDxWD*x_ds_rgbaff_BxC2xHDxWD[:, -3:-2, :, :]
        ...
        if self.in_channels == 4:
            y_pred_gs_Bx1xHSxWS = y_pred_gs_Bx1xHSxWS * x_gs_rgbaff_BxC2xHSxWS[:, -3:-2, :, :]

        y_pred_BxK = self.gen_cls(self.transform_rgbaff(x_gs_rgbaff_BxC2xHSxWS) * y_pred_gs_Bx1xHSxWS)
