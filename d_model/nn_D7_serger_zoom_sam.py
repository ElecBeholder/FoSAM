...
from d_model.nn_B12_EfficientSAM import CustomSAM


class SegerZoomSAM(nn.Module):
    ...
    def __init__(self,
                 ...
                 square_focus=True,
                 ckpt_path = None):
        super(SegerZoomSAM, self).__init__()
        ...
        self.transform_rgba = transforms.Compose([
            transforms.Normalize(mean=[0.485, 0.456, 0.406, 0.5], std=[0.229, 0.224, 0.225, 0.25])  # 针对 ImageNet 进行标准化
        ])
    ...
    def forward(self, x_BxCxHxW: torch.Tensor, x_Bx2: torch.Tensor):
        ...
        normalized_dist_BxHxW = (dist_BxHxW - min_dist) / (max_dist - min_dist + 1e-6)

        # Invert it so closest points are 1, farthest points are 0
        focusmap_Bx1xHxW = normalized_dist_BxHxW.unsqueeze(1)
        
        # focusmap_Bx1xHxW = (dist_BxHxW / max_dist).unsqueeze(1)
        # focusmap_Bx1xHxW = torch.zeros((B, 1, HS, WS), device=x_BxCxHxW.device)
        
        # one-hot embed the focus point
        # focusmap_Bx1xHxW = torch.zeros((B, 1, HS, WS), device=x_BxCxHxW.device)
        # focusmap_Bx1xHxW[:, 0, hidx_B.long(), widx_B.long()] = 1.0
        ...
        x_gs_rgba = x_gs_rgbaf_BxC1xHSxWS[:, :-1, :, :]

        x_Bx2 = x_Bx2.unsqueeze(1)
        output_shape = (x_Bx2.shape[0],1,1)

        out1_Bx1xHSxWS = self.gen_seg(images = self.transform_rgba(x_gs_rgba), batched_points = (x_Bx2*(HS-1)).unsqueeze(1), batched_point_labels = (torch.ones(output_shape)).to(target_device) )
