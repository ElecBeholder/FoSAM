...
class PreprocessCityscapes:
    ...
    @watch_time
    def __init__(self, dataset_partition='train', cropH=512, cropW=512):
    ...
    def make_a_sample(self, itemkey, idx_H, idx_W, fpath_X, fpath_Y, oidx_target, mark: str = 'default'):
        ...
        label_1xHSxWS = labelidx_tracker_HxW[h_sidx:h_eidx, w_sidx:w_eidx].unsqueeze(0)
        ...
        fname_S = f"{sent_catyname}_{sent_cid}_{sent_kid}_{sent_itemkey}_{sent_fpos}_4x{sent_csize}.uint8.XY.png"

        XY_rgba_4xHSxWS = add_alpha(view_rgb_3xHSxWS)
        XY_rgba_4xHSxWS[-1, :, :] = label_1xHSxWS[0]

        save_image(XY_rgba_4xHSxWS, os.path.join(self.path_data_cook_cityscapes_part, mark, fname_S))
    ...
    def make_N_samples(self, N, marker):
        ...
        for n in trange(N):
            ...
            self.make_a_sample_by_label(wrap_name(target_label), mark=marker)
...
class DatasetCityScapes(AbstractDataset):
    ...
    def __init__(self, marker, dataset_partition='train', cropH=512, cropW=512):
        super().__init__()
        self.cropH = cropH
        self.cropW = cropW
    ...
    def get_fnames_Ypt(self):
        return [fname for fname in os.listdir(self.path_data_cook_cityscapes_part_mark) if fname.endswith('.XY.png')]
    ...
    def __getitem__(self, index):
        ...
        CxHxW_X = '4x' + IxHxW_Y[2:]
        fname_XY = f"{caty}_{cid}_{kid}_{itemkey}_{fpos}_{CxHxW_X}.uint8.XY.png"

        Y_cls_s = int(kid[1:])

        fpath_XY = os.path.join(self.path_data_cook_cityscapes_part_mark, fname_XY)
        XY_4xHxW = load_image(fpath_XY, mode='RGBA')
        X_3xHxW = XY_4xHxW[:3, :, :]
        Y_1xHxW = XY_4xHxW[3:, :, :]

        # plt_multi_imgshow([XY_4xHxW[:3], XY_4xHxW[3:]])
        # plt_show()
...
if __name__ == '__main__':
    ...
    if Skwargs.task == 'preprocess':
        ...
        w = Watch()
        HW_size = Skwargs.HW_size
        pplv_train = PreprocessCityscapes(dataset_partition='train', cropH=HW_size, cropW=HW_size)
        pplv_valid = PreprocessCityscapes(dataset_partition='valid', cropH=HW_size, cropW=HW_size)
        ...
        for dp in Skwargs.dataset_partition:
            for sp_train in Skwargs.sample_num:
                ...
                datasetCityscapes = DatasetCityScapes(marker, dataset_partition=dp, cropH=HW_size, cropW=HW_size)
...
python e_preprocess_scripts/b3_preprocess_cityscapes.py --task preprocess --dataset_partition train valid --sample_num 60000 --HW_size 1024 --marker_prefix sp1024_

python e_preprocess_scripts/b3_preprocess_cityscapes.py --task speed_test --dataset_partition train --sample_num 60 --epoch 5 --batch_size 64 --target_device gpu --HW_size 1024 --marker_prefix sp1024_


python e_preprocess_scripts/b3_preprocess_cityscapes.py --task preprocess --dataset_partition train valid --sample_num 60000 --HW_size 1024 --marker_prefix sp1024_

python e_preprocess_scripts/b3_preprocess_cityscapes.py --task preprocess --dataset_partition train valid --sample_num 200 --HW_size 256 --marker_prefix sp256_
python e_preprocess_scripts/b3_preprocess_cityscapes.py --task preprocess --dataset_partition train valid --sample_num 200 --HW_size 128 --marker_prefix sp128_
python e_preprocess_scripts/b3_preprocess_cityscapes.py --task preprocess --dataset_partition train valid --sample_num 200 --HW_size 64 --marker_prefix sp64_
python e_preprocess_scripts/b3_preprocess_cityscapes.py --task preprocess --dataset_partition train valid --sample_num 200 --HW_size 32 --marker_prefix sp32_




on local
python e_preprocess_scripts/b3_preprocess_cityscapes.py --task preprocess --dataset_partition train valid --sample_num 12000
...
python e_preprocess_scripts/b3_preprocess_cityscapes.py --delete --dataset_partition train valid --sample_num 10000 2000 --marker_prefix 1024
