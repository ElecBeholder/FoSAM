...
class PreprocessCityscape(AbstractDataset):
    ...
    def __init__(self, dataset_partition='train', use_cache=True):
        self.dataset_partition = dataset_partition

        self.canvas_H = 1024
        self.canvas_W = 1024
        self.HD = self.canvas_H
        self.WD = self.canvas_W
    ...
    def make_a_sample(self, itemkey, idx_H, idx_W, fpath_X, fpath_Y, oidx_target):
        ...
        labelidx_tracker_pil = transforms.ToPILImage()(labelidx_tracker_HxW)
        ...
        original_width, original_height = view_rgb_pil.size

        top_min = max(0, idx_H - self.HD + 1)
        top_max = min(idx_H, original_height - self.HD)
        top = random.randint(top_min, top_max)

        left_min = max(0, idx_W - self.WD + 1)
        left_max = min(idx_W, original_width - self.WD)
        ...
        cropped_image_pil = crop_transform(view_rgb_pil, top, left, self.HD, self.WD)
        cropped_labelidx_pil = crop_transform(labelidx_tracker_pil, top, left, self.HD, self.WD)


        view_rgb_3xHSxWS = transforms.ToTensor()(cropped_image_pil)
        labelidx_HSxWS = transforms.ToTensor()(cropped_labelidx_pil).squeeze(0)  # Remove channel dimension for label
...
def main():
    ...
    parser.add_argument(
        '--samples_per_class', type=int, required=False, default=100,
    ...
    for dp in args.dataset_partition:
        print(f"dataset_partition='{dp}', samples_per_class={args.samples_per_class}, clean='{args.clean}', use_cache={args.use_cache}")
