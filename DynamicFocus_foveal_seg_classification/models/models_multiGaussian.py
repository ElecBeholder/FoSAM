...
class DeformSegmentationModule(nn.Module):
    def __init__(self, cfg):
        ...
        # 直接使用ViT版本的多高斯预测器
        self.gaussian_predictor = GaussianPredictor_multiGaussian_ViT(40, 384)
        print("使用Vision Transformer版本的多高斯预测器")
    ...
    def forward(self, img_data, img_original, focus_point, s_bin_selected_BxHMxWMx1, segSize=None,):
        ...
        pred = self.gaussian_predictor(x, normalized_gaze_coords)
        # 使用多高斯显著性图生成函数
        xs = generate_saliency_map_multiGaussian(pred, H_HS, W_HS)

        # 保存所有45个高斯参数
        self.gaussian_pred_Bx7 = pred  # 虽然变量名仍为Bx7，实际上是Bx45
        self.xs = xs
        xs = xs.detach()
