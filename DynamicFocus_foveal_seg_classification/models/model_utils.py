...
class C1(nn.Module):
    def __init__(self, num_class=150, fc_dim=2048, use_softmax=False):
        ...
        self.cls_net = ResNet(block = ResidualBlock)
    ...
    def forward(self, conv_out, segSize=None, res=None):
        ...
        return cls_pred_new
        #return cls_pred[:, :, None, None] * x.repeat(1, 20, 1, 1).cuda()
        #return cls_pred[:, :, None, None] *  x



        '''
        # then we will merge the results
        cls_pred = self.cls_net(conv5)   # [1,19]
        x = nn.functional.sigmoid(x)
        mask_fg = (x>0.5).float().cuda()
        mask_bg = 1-mask_fg
        
        #print(mask_fg.shape)   #torch.Size([1, 2, 64, 128])
        #mask_fg = torch.stack([mask_fg]*20, dim=2).cuda()
        #print(mask_fg.shape)
        mask_fg = mask_fg.repeat(1, 20, 1, 1).cuda()   # (1, 20, 64, 128)
        cls_pred = torch.cat([torch.zeros(1,1).cuda(),cls_pred],dim=1)[:, :, None, None].cuda()

        #print(mask_fg.shape, cls_pred.shape)  # torch.Size([1, 20, 64, 128]) torch.Size([1, 20])
        result = mask_fg*cls_pred
        result[0,0, :, :] = (mask_bg * x)[0,0,:]
        #print(result[0,0, :, :])
        #print(cls_pred)
        return result
        '''
