...
def create_optimizers(nets, cfg):
    ...
    if cfg.TRAIN.optim.lower() == 'sgd':
        ...
        # #segformer b3 optimizer
        # param_groups = [
        #     #{'params': net_encoder.parameters(), 'lr': 0.00006, 'weight_decay': 0.01},  # Default group
        #     {'params': [p for n, p in net_encoder.named_parameters() if ('pos_block'  not in n) and ('norm'  not in n) and ('head'  not in n)], 'lr': 0.00006, 'weight_decay': 0.01,'zoom':False,'lr_mult':cfg.TRAIN.lr_mult_encoder},
        #     {'params': [p for n, p in net_encoder.named_parameters() if 'pos_block' in n], 'weight_decay': 0.0,'zoom':False,'lr_mult':cfg.TRAIN.lr_mult_encoder},  # No weight decay for pos_block
        #     {'params': [p for n, p in net_encoder.named_parameters() if 'norm' in n and 'pos_block' not in n], 'weight_decay': 0.0,'zoom':False,'lr_mult':cfg.TRAIN.lr_mult_encoder},  # No weight decay for normalization layers
        #     {'params': [p for n, p in net_encoder.named_parameters() if 'head' in n and 'pos_block' not in n and 'norm' not in n], 'lr': cfg.TRAIN.lr_encoder * 10,'zoom':False,'lr_mult':cfg.TRAIN.lr_mult_encoder}  # 10x LR for head
        # ]
