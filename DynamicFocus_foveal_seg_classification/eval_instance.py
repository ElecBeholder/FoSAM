...
def eval_during_train_deform(cfg, writer=None, gpu=0, count=None, dataset_marker_val='sp2000'):
    ...
    datasetloader_val = CustomDataLoader(DatasetCityScapes(marker=dataset_marker_val, dataset_partition='valid'), cache=True, xrange=trange)
    iterator_val = enumerate(datasetloader_val.get_iterator(batch_size=cfg.VAL.batch_size, device='cuda', shuffle=True))
    loader_val = list(iterator_val)
