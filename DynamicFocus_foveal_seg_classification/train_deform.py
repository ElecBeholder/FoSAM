...
# train one epoch
def train(segmentation_module, datasetloader_train, optimizers, epoch, cfg, history=None, foveation_module=None, writer=None):
    ...
    #cfg.TRAIN.epoch_iters = 744
    #for i in range(cfg.TRAIN.epoch_iters):
    for i, data in enumerate(datasetloader_train.get_iterator(batch_size=cfg.TRAIN.batch_size_per_gpu, device='cuda', shuffle=True)):
        ...
        print_grad = None
        cfg.TRAIN.deform_joint_loss = False   # do not use edge loss for now
        if single_gpu_mode:
...
def adjust_learning_rate(optimizers, cur_iter, cfg, lr_mbs = False, f_max_iter=1, lr_scale=1, wd_scale=1, epoch=None):
    ...
    if not lr_mbs:
        ...
        #print('ddddddddddddddd', float(cur_iter), cfg.TRAIN.max_iters)
    ...
    #print('xxxxx', cfg.TRAIN.running_lr_encoder)
    #print('yyyyy', cfg.TRAIN.lr_encoder)
    #print('zzzzz', scale_running_lr)
...
def main(cfg, gpus):
    ...
    #loader_train = DataLoaderCityScapesRGBLabelMask(datasetCityscapes)

    #datasetCityscapes = DatasetCityScapes('sp0', dataset_partition='train')
    #loader_train = DataLoaderCityScapes(datasetCityscapes)

    #Cityscape b3 dataloader
    print(cfg.TRAIN.batch_size_per_gpu)
    cfg.TRAIN.batch_size_per_gpu = 1
    datasetloader_train = CustomDataLoader(DatasetCityScapes(marker=cfg.DATASET.dataset_marker_train, dataset_partition='train'), cache=True, xrange=trange)

    # create loader iterator
    iterator_train = enumerate(datasetloader_train.get_iterator(batch_size=cfg.TRAIN.batch_size_per_gpu, device='cuda', shuffle=True))
    #iterator_train = enumerate(loader_train.get_iterator(batch_size=20, device='cuda'))
    ...
    for epoch in range(cfg.TRAIN.start_epoch, cfg.TRAIN.num_epoch):
        ...
        if not cfg.TRAIN.skip_train_for_eval:
            ...
            train(segmentation_module, datasetloader_train,
        ...
        if (epoch+1) % cfg.TRAIN.eval_per_epoch == 0:
            if cfg.VAL.dice:
                if cfg.VAL.y_sampled_reverse:
                    val_iou, val_dice, val_acc, val_iou_deformed, val_dice_deformed, val_acc_deformed, val_iou_y_reverse, val_dice_y_reverse, val_acc_y_reverse, relative_eval_y_ysample, ious = eval_during_train_deform(cfg, writer=writer, count=epoch+1, dataset_marker_val=cfg.DATASET.dataset_marker_valid)
                else:
                    val_iou, val_dice, val_acc, val_iou_deformed, val_dice_deformed, val_acc_deformed, relative_eval_y_ysample, ious = eval_during_train_deform(cfg, writer=writer, count=epoch+1, dataset_marker_val=cfg.DATASET.dataset_marker_valid)
            else:
                if cfg.VAL.y_sampled_reverse:
                    val_iou, val_acc, val_iou_deformed, val_acc_deformed, val_iou_y_reverse, val_acc_y_reverse, relative_eval_y_ysample, ious = eval_during_train_deform(cfg, writer=writer, count=epoch+1, dataset_marker_val=cfg.DATASET.dataset_marker_valid)
                else:
                    val_iou, val_acc, val_iou_deformed, val_acc_deformed, relative_eval_y_ysample, ious = eval_during_train_deform(cfg, writer=writer, count=epoch+1, dataset_marker_val=cfg.DATASET.dataset_marker_valid)
            # deform y-y_sampled
...
if __name__ == '__main__':
    ...
    cfg.TRAIN.epoch_iters = int(cfg.DATASET.dataset_marker_train[2:])
